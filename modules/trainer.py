        ...
        with torch.no_grad():
            ...
            for batch_idx, (images_id, images, reports_ids, reports_masks) in enumerate(tqdm(self.test_dataloader)):
                ...
                output = self.model(images, mode='sample')
                 ...
                 # Determine the actual model to access attributes like 'tokenizer'
                if isinstance(self.model, torch.nn.parallel.DistributedDataParallel):
                    actual_model = self.model.module
                else:
                    actual_model = self.model

                reports = actual_model.tokenizer.decode_batch(output.cpu().numpy())
                ground_truths = actual_model.tokenizer.decode_batch(reports_ids[:, 1:].cpu().numpy())
